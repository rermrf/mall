...
export async function batchGetStock(skuIds: number[]) {
  return request<Inventory[]>({ method: 'POST', url: '/inventory/stock/batch', data: { skuIds: skuIds } })
}

export async function listInventoryLogs(params: { skuId?: number; page?: number; pageSize?: number }) {
