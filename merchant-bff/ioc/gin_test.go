...
func issueMerchantRefreshToken(t *testing.T, jwtHandler *ijwt.JWTHandler) string {
	t.Helper()
	...
	if err := jwtHandler.SetTokenHeaders(ctx, 123, 8); err != nil {
		t.Fatalf("issue merchant tokens: %v", err)
	}
...
func TestRefreshRouteAllowsRefreshTokenWithoutAccessToken(t *testing.T) {
	...
	refreshToken := issueMerchantRefreshToken(t, jwtHandler)
