...
func (b *LoginJWTBuilder) Build() gin.HandlerFunc {
	return func(ctx *gin.Context) {
		...
		if authHeader == "" {
			ctx.AbortWithStatusJSON(http.StatusUnauthorized, ginx.Result{Code: 401001, Msg: "未登录"})
			return
		}
		parts := strings.SplitN(authHeader, " ", 2)
		if len(parts) != 2 || parts[0] != "Bearer" {
			ctx.AbortWithStatusJSON(http.StatusUnauthorized, ginx.Result{Code: 401001, Msg: "token 格式错误"})
			return
		}
		claims, err := b.jwtHandler.ParseAccessToken(parts[1])
		if err != nil {
			ctx.AbortWithStatusJSON(http.StatusUnauthorized, ginx.Result{Code: 401001, Msg: "token 无效或已过期"})
			return
		}
